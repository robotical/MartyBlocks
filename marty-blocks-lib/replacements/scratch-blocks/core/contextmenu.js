...
/**
 * Make a context menu option for transpiling the current block to Python.
 * @param {!Blockly.BlockSvg} block The block where the right-click originated.
 * @param {!Event} event Event that caused the context menu to open.
 * @return {!Object} A menu option, containing text, enabled, and a callback.
 * @package
 */
Blockly.ContextMenu.blockTranspileToPythonOption = function(block, event) {
  var transpileOption = {
    text: Blockly.Msg.PYTHON_TRANSPILE,
    enabled: true,
    callback: () => {
      // get root block
      ...
      // keep only the blocks that are relevant to this block and its children
      target.blocks = Object.keys(target.blocks).reduce((acc, blockId) => {
        if (childrenIds.includes(blockId)) {
          acc[blockId] = target.blocks[blockId];
        }
        return acc;
      }, {});
