...
Blockly.Flyout.prototype.addBlockListeners_ = function(root, block, rect) {
  ...
  this.listeners_.push(Blockly.bindEvent_(root, 'mouseover', block,
      this.blockMouseOver_(block)));
  this.listeners_.push(Blockly.bindEvent_(root, 'mouseout', block,
      this.blockMouseOut_(block)));
...
/**
 * Handle a mouse-out event on a block in the flyout.
 * @param {!Blockly.Block} block The block the mouse is over.
 * @return {!Function} Function to call when block is moused out.
 * @private
 */
Blockly.Flyout.prototype.blockMouseOut_ = function(block) {
  var flyout = this;
  return function(e) {
    // TODO: show full block on hover
    // block.translate(newXY.x, newXY.y);
    // flyout.targetWorkspace_.blockDragSurface_.clearAndHide(flyout.targetWorkspace_.getCanvas());
    block.removeSelect();
  };
};

/**
 * Handle a mouse-over on an SVG block in a non-closing flyout.
 * @param {!Blockly.Block} block The flyout block to mouse over.
 * @return {!Function} Function to call when block is moused over.
 * @private
 */
Blockly.Flyout.prototype.blockMouseOver_ = function(block) {
  var flyout = this;
  return function(e) {
    // TODO: show full block on hover
    // var x = 0;
    // var y = 0;
    // var dragSurfaceGroup = flyout.targetWorkspace_.blockDragSurface_.getGroup();
  
    // var element = block.getSvgRoot();
    // if (element) {
    //   do {
    ...
    //     if (flyout.targetWorkspace_.blockDragSurface_.getCurrentBlock() == element) {
    //       var surfaceTranslation = flyout.targetWorkspace_.blockDragSurface_.getSurfaceTranslation();
    //       x += surfaceTranslation.x;
    //       y += surfaceTranslation.y;
    //     }
    //     element = element.parentNode;
    //   } while (element && element.getAttribute && element != flyout.targetWorkspace_.getCanvas() &&
    //       element != dragSurfaceGroup);
    // }
    // const relativeToSurfaceXY = new goog.math.Coordinate(x, y);
    // Blockly.utils.removeAttribute(block.getSvgRoot(), 'transform');

    // flyout.targetWorkspace_.blockDragSurface_.translateSurface(relativeToSurfaceXY.x, relativeToSurfaceXY.y);

    // // Execute the move on the top-level SVG component
    // flyout.targetWorkspace_.blockDragSurface_.setBlocksAndShow(block.getSvgRoot());
  block.addSelect();
  }
}
