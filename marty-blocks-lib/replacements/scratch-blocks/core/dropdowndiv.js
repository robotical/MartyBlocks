...
Blockly.DropDownDiv.getPositionMetrics = function(primaryX, primaryY, secondaryX, secondaryY) {
  ...
  if (primaryY + divSize.height > boundPosition.top + boundSize.height) {
    ...
    renderX = primaryX;
    renderY = primaryY + Blockly.DropDownDiv.PADDING_Y;
    if (boundSize.height < 600) {
      // fixing M20-1488 where keypad behaves weird on small phones.
      // if boundSize height is smaller than 600, implies that we're on a small screen device like iphone 6 
      // following if statements decrease the padding between the block and the keypad.
      if (primaryY > 185 && primaryY < 215) {
        renderY = primaryY - Blockly.DropDownDiv.PADDING_Y;
      } else if (primaryY >= 215 && primaryY < 235) {
        renderY = primaryY - Blockly.DropDownDiv.PADDING_Y * 2;
      } else if (primaryY >= 235 && primaryY < 250) {
        renderY = primaryY - Blockly.DropDownDiv.PADDING_Y * 3;
      } else if (primaryY >= 250) {
        renderY = primaryY - Blockly.DropDownDiv.PADDING_Y * 4;
      }
    }
    renderedSecondary = false;
  }
