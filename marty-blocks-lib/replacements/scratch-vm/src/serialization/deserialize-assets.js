...
/**
 * Deserializes model from file into storage cache so that it can
 * be loaded into the runtime.
 * @param {object} model Descriptor for model from sb3 file
 * @param {Runtime} runtime The runtime containing the storage to cache the models in
 * @param {JSZip} zip The zip containing the model file being described by `model`
 * @param {string} assetFileName Optional file name for the given asset
 * (sb2 don't have models, sb3 files have filenames of the form [md5].[ext])
 * @return {Promise} Promise that resolves after the described model has been stored
 */
const deserializeModel = function (model, runtime, zip, assetFileName) {
    const fileName = assetFileName ? assetFileName : model.md5;
    const storage = runtime.storage;
    if (!storage) {
        log.warn('No storage module present; cannot load model asset: ', fileName);
        return Promise.resolve(null);
    }
    ...
    let modelFile = zip.file(fileName);
    if (!modelFile) {
        // look for assetfile in a flat list of files, or in a folder
        const fileMatch = new RegExp(`^([^/]*/)?${fileName}$`);
        modelFile = zip.file(fileMatch)[0]; // use first matching file
    }

    if (!modelFile) {
        log.error(`Could not find model file associated with the ${model.name} model.`);
        return Promise.resolve(null);
    }
    ...
    return modelFile.async('uint8array')
        .then(data => {
            // need to de-flatten the weights
            const weightsInfo = model.dependencies[1];
            const longArrayFloat32 = new Float32Array(data.buffer);
        ...
        .then(asset => {
            ...
            model.md5 = `${asset.assetId}.${asset.dataFormat}`;
        });
};
