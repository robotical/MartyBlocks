...
class VirtualMachine extends EventEmitter {
    ...
    /**
     * Add a device. Unpack and validate first.
     * @param {string | object} input A json string, object, or ArrayBuffer representing the project to load.
     * @return {!Promise} Promise that resolves after targets are installed.
     */
    addDevice(input) {
        const errorPrefix = 'Device Upload Error:';
        if (typeof input === 'object' && !(input instanceof ArrayBuffer) &&
            !ArrayBuffer.isView(input)) {
            // If the input is an object and not any ArrayBuffer
            // or an ArrayBuffer view (this includes all typed arrays and DataViews)
    ...
    /**
     * Add a single device.
     * @param {object} sprite Object rperesenting device to be added.
     * @param {?ArrayBuffer} zip Optional zip of assets being referenced by target json
     * @returns {Promise} Promise that resolves after the sprite is added
     */
    ...
    /**
     * Add a model to the current editing target.
     * @param {!object} modelObject Object representing the costume.
     * @param {string} optTargetId - the id of the target to add to, if not the editing target.
     * @returns {?Promise} - a promise that resolves when the model has been decoded and added
     */
    addModel(modelObject, optTargetId) {
        const target = optTargetId ? this.runtime.getTargetById(optTargetId) :
            this.editingTarget;
        if (target) {
    ...
    /**
     * Delete a device and all its clones.
     * @param {string} targetId ID of a target whose device to delete.
     * @return {Function} Returns a function to restore the device that was deleted
     */
