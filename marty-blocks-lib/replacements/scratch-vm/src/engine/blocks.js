    ...
    changeBlock(args) {
        ...
        switch (args.element) {
            case 'field':
                ...
                const oldValue = block.fields[args.name].value;
                if (args.name === 'VARIABLE' || args.name === 'LIST' ||
                    ...
                    const idOfCurrentlySelectedBlock = getMonitorIdForBlockWithArgs(block.id, block.fields);
                    const currentlySelectedBlock = this.runtime.monitorBlocks.getBlock(idOfCurrentlySelectedBlock);
                    // const idOfPreviouslySelectedBlock = getMonitorIdForBlockWithArgs(block.id, { [args.name]: oldValue });
                    // const previouslySelectedBlock = this.runtime.monitorBlocks.getBlock(idOfPreviouslySelectedBlock);
                    const flyout = window.workspace.getFlyout();
                    if (!currentlySelectedBlock) {
                        // if the currently selected block is not a monitor block, uncheck the checkbox
                        flyout.setCheckboxState(flyoutBlock.id, false);
                    } else if (currentlySelectedBlock && currentlySelectedBlock.isMonitored) {
                        // if the currently selected block is a monitor block, check the checkbox
                        flyout.setCheckboxState(flyoutBlock.id, true);
                    } else if (currentlySelectedBlock && !currentlySelectedBlock.isMonitored) {
                        // if the currently selected block is not monitored, uncheck the checkbox
                        flyout.setCheckboxState(flyoutBlock.id, false);
                    }
                }
                break;
