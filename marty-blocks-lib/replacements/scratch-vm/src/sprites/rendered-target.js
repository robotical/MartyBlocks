...
class RenderedTarget extends Target {
    ...
    /**
     * Add a model, taking care to avoid duplicate names.
     * @param {!object} model the model to be added
     * @param {?int} index Index at which to add model
     */
    addModel(model, index) {
        if (typeof index === 'number' && !isNaN(index)) {
            this.sprite.models.splice(index, 0, model);
        } else {
    ...
    /**
     * Delete Model by index.
     * @param {number} index Model index to be deleted
     * @return {object} The deleted model object, or null if no model was deleted.
     */
    deleteModel(index) {
        // Make sure the model index is not out of bounds
        if (index < 0 || index >= this.sprite.models.length) {
            return null;
        }
        // Delete the model at the given index
        const deletedModel = this.sprite.models.splice(index, 1)[0];
        this.runtime.requestTargetsUpdate(this);
        return deletedModel;
    }
