...
class Scratch3Text2SpeechBlocks {
  ...
  getInfo() {
    ...
    return {
      ...
      blocks: [
        ...
        {
          opcode: "axiom_speakAndWait",
          blockIconURI: axiomBlockIconURI,
          colour: "#5ba591",
          colourSecondary: "#498474",
          text: formatMessage({
            id: "text2speech.axiom_speakAndWaitBlock",
            default: "Axiom speak [WORDS]",
            description: "Speak some words.",
          }),
          blockType: BlockType.COMMAND,
          arguments: {
   ...
   * @return {Promise} A promise that resolves after playing the sound
   */
  axiom_speakAndWait(args, util) {
    return this.speakHelper(args, util, 'axiom');
  }
        ...
        .then((decodedBuffer) => {
          ...
          offlineContext
            ...
            .then((renderedBuffer) => {
              ...
              } else if (blockType === 'axiom')  {
                if (axiomInterface.isConnected) {
                  // axiomInterface.streamAudio(mp3SoundData, maxDuration * 1000);
                } else {
                  window.vm.runtime.stopAll();
                  // return axiomInterface.send_REST(
                  //   "notification/warn-message/You are not currently connected to a Marty. Please connect."
                  // );
                }
              } else {
